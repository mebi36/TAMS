...
def face_enrol():
    ...
    #continuously capture video frame by frame
    while (True):
        ...
        # wait for key press
        key = cv2.waitKey(10)
        # set 'c' to capture image
        if  key & 0xFF == ord('c'):
            os.makedirs(os.path.dirname(f"{reg_number}/"), exist_ok=True)
            cv2.imwrite(f"{reg_number}/{reg_number}.png", frame)
...
def face_verify():
    ...
    # continuously capture webcam snapshots
    while True:
        result, frame = cam.read()

        # Display the resulting frame
        cv2.imshow('Student Face Capture', frame)
